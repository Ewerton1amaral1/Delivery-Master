...
export const StoreApp: React.FC<StoreAppProps> = ({ storeId, onLogout }) => {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'orders' | 'clients' | 'products' | 'pricing' | 'supplies' | 'settings' | 'reports' | 'drivers' | 'team'>('dashboard');
  ...
  const [drivers, setDrivers] = useState<Driver[]>(() => {
    const saved = localStorage.getItem(getKey('drivers'));
    return saved ? JSON.parse(saved) : [];
  });
  const [employees, setEmployees] = useState<Employee[]>(() => {
    const saved = localStorage.getItem(getKey('employees'));
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const NavItem = ({ id, label, icon: Icon }: { id: typeof activeTab, label: string, icon: any }) => (
    <button
      ...
      className={`flex items-center gap-3 px-4 py-3 w-full rounded-xl transition font-medium ${
        activeTab === id 
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col md:flex-row font-sans text-gray-900 dark:text-gray-100 transition-colors duration-200">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-200">
        ...
        {activeTab === 'orders' && <OrderManagement orders={orders} setOrders={setOrders} settings={storeSettings} drivers={drivers} />}
        ...
        {activeTab === 'settings' && <Settings settings={storeSettings} onSave={setStoreSettings} onLogout={onLogout} />}
      </main>
