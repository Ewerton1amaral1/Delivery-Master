...
export const TeamManagement: React.FC<TeamManagementProps> = ({ employees, setEmployees }) => {
  ...
  const handleSave = () => {
    ...
    if (currentEmployee.id) {
      ...
      const newEmp: Employee = {
        ...
        transportVoucherValue: currentEmployee.transportVoucherValue || 0
      } as Employee;
  ...
  const addAdvance = () => {
    if (!newAdvance.amount || !currentEmployee.advances) return;
    
    const advance: EmployeeAdvance = {
      id: Date.now().toString(),
      date: new Date().toISOString(),
      amount: parseFloat(newAdvance.amount),
      description: newAdvance.desc || 'Adiantamento/Vale'
    };

    const updatedAdvances = [...(currentEmployee.advances || []), advance];
    setCurrentEmployee({ ...currentEmployee, advances: updatedAdvances });
    setNewAdvance({ amount: '', desc: '' });
  };

  const removeAdvance = (advId: string) => {
    const updatedAdvances = currentEmployee.advances?.filter(a => a.id !== advId) || [];
    setCurrentEmployee({ ...currentEmployee, advances: updatedAdvances });
  };
