...
export const ProductManagement: React.FC<ProductManagementProps> = ({ products, setProducts }) => {
  ...
  const handleSave = () => {
    ...
    if (currentProduct.id) {
      ...
      setProducts(prev => [...prev, { ...currentProduct, id: Date.now().toString(), imageUrl: `https://picsum.photos/200/200?random=${Date.now()}` } as Product]);
    }
    setIsEditing(false);
    setCurrentProduct({});
  };
  ...
  const requestDelete = (e: React.MouseEvent, id: string) => {
    e.stopPropagation(); // Critical: Prevent the card click event
