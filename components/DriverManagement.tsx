...
export const DriverManagement: React.FC<DriverManagementProps> = ({ drivers, setDrivers, orders, setOrders }) => {
  ...
  const handleSave = () => {
    ...
    if (currentDriver.id) {
      setDrivers(prev => prev.map(d => d.id === currentDriver.id ? currentDriver as Driver : d));
    } else {
      setDrivers(prev => [...prev, { ...currentDriver, id: Date.now().toString(), isActive: true } as Driver]);
    }
    setIsEditing(false);
    setCurrentDriver({});
  };
